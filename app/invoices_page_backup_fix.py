...
def render_invoice_list(invoices, selected_index=None, on_select=None):
    ...
    Features:
        </ul>- Real-time status updates with dynamic database loading
            ...
            if line_items:
                ...
                for idx, item in enumerate(line_items):
                    ...
                    if has_discrepancy:
                        ...
                        discrepancy_value_html = (
                            f'<div style="font-size: 0.7rem; color: #666; margin-top: 0.2rem;">{format_currency(discrepancy_value)}</div>'
                        ) if discrepancy_value > 0 else ''
                        status_cell = f'''
                ...
                if discrepancy_count > 0:
                    discrepancy_html = f"<div style='background: #fff7e0; color: #b8860b; padding: 0.4rem 0.8rem; border-radius: 4px; font-size: 0.8rem; font-weight: 600; border: 1px solid #f1c232;'>{discrepancy_count} Issues</div>"
                else:
                    discrepancy_html = "<div style='background: #f0fdf4; color: #10b981; padding: 0.4rem 0.8rem; border-radius: 4px; font-size: 0.8rem; font-weight: 600; border: 1px solid #10b981;'>✅ Clean</div>"
                st.markdown(f'''
...
def render_issues_detected_box(invoices, flagged_issues=None):
    ...
    try:
        ...
        if flagged_issues:
            ...
            over_delivered_html = (
                f"<div style='background: #fff3cd; border: 1px solid #f1c232; border-radius: 6px; padding: 0.5rem 1rem; flex: 1; min-width: 150px;'>"
                f"<div style='font-weight: 600; color: #b8860b; font-size: 1rem;'>{len(over_delivered_issues)}</div>"
                "<div style='font-size: 0.8rem; color: #666;'>Over Delivered</div>"
                "</div>"
            ) if over_delivered_issues else ''
            under_delivered_html = (
                f"<div style='background: #fff3cd; border: 1px solid #f1c232; border-radius: 6px; padding: 0.5rem 1rem; flex: 1; min-width: 150px;'>"
                f"<div style='font-weight: 600; color: #b8860b; font-size: 1rem;'>{len(under_delivered_issues)}</div>"
                "<div style='font-size: 0.8rem; color: #666;'>Under Delivered</div>"
                "</div>"
            ) if under_delivered_issues else ''
            flagged_items_html = (
                f"<div style='background: #fff5f5; border: 1px solid #ff3b30; border-radius: 6px; padding: 0.5rem 1rem; flex: 1; min-width: 150px;'>"
                f"<div style='font-weight: 600; color: #ff3b30; font-size: 1rem;'>{len(flagged_items)}</div>"
            ...
            for idx, issue in enumerate(flagged_issues):
                ...
                # Prepare conditional HTML
                item_id_html = (
                    f"<div style='font-size: 0.8rem; color: #999; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 0.3rem;'>"
                    f"Item ID: {issue['item_id']}"
                    "</div>"
...
def render_footer_buttons(on_clear=None, on_submit=None, disabled=False, show_loading=False):
    ...
    try:
        ...
        st.markdown(f'''
            <div class="owlin-footer-buttons" style="background: #f8f9fa; border-top: 2px solid #e9ecef; padding: 2rem 0; margin-top: 3rem;">
                <div style="max-width: 1200px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 2rem;">
                    ...
                    <div style="flex: 1; min-width: 300px;">
                        <div style="font-weight: 700; color: #222; font-size: 1.2rem; margin-bottom: 0.5rem;">
                            Ready to Submit
                        </div>
                        <div style="color: #666; font-size: 0.9rem; line-height: 1.4;">
                    ...
                    <div class="owlin-not-paired-item {urgency_class}" 
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem; flex-wrap: wrap; gap: 1rem;">
                            <div style="flex: 1; min-width: 250px;">
                                ...
                                {f"<div style='font-size: 0.8rem; color: #999; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 0.3rem;'>Invoice ID: {invoice_id}</div>" if invoice_id else ''}
                            </div>
...
def render_invoices_page():
    """Render the complete invoices page with all interactive components."""
    ...
    def submit_to_owlin():
        ...
        try:
            ...
            if unpaired_count > 0:
                st.warning(
                    f"⚠️ {unpaired_count} invoice(s) are not paired with delivery notes. Please pair them before submitting."
                )
            ...
            for inv in invoices:
                ...
                if details and details.get("line_items"):
                    for item in details.get("line_items", []):
                        if item.get("flagged") or (
                            item.get("delivery_qty") is not None
                            and item.get("delivery_qty") != item.get("invoice_qty", 0)
                        ):
                            flagged_issues.append(item)

            if flagged_issues:
                st.warning(
                    f"⚠️ {len(flagged_issues)} issue(s) detected. Please resolve them before submitting."
                )
            ...
            total_value = sum(inv.get("total", 0) for inv in invoices)
            return (
                f"Successfully submitted {len(invoices)} invoices worth {format_currency(total_value)}"
            )
    ...
    if not invoices:
        st.info(
            "💡 **Getting Started:** Upload invoices and delivery notes to begin processing. The system will automatically detect discrepancies and help you manage your hospitality invoices efficiently."
        )

    if "processing_files" in st.session_state and st.session_state.processing_files:
        st.info(
            "🔄 Some files are still being processed. Please wait for completion before proceeding."
        )
    ...
    if st.checkbox(
        ...
        key="show_shortcuts",
        help="Display available keyboard shortcuts",
    ):
        st.markdown(
            """
...
def clear_temp_session_state():
    ...
    try:
        ...
        # Clean up error tracking keys
        error_keys = [k for k in st.session_state.keys() if k.endswith('_error_count')]
        for key in error_keys:
            del st.session_state[key]
        
        # Clean up loading tracking keys
        loading_keys = [k for k in st.session_state.keys() if k.endswith('_load_time')]
        for key in loading_keys:
            del st.session_state[key]
        
        # Clean up display tracking keys
        display_keys = [k for k in st.session_state.keys() if k.endswith('_display_count')]
        for key in display_keys:
            del st.session_state[key]
        
    except Exception as e:
        st.error(f"❌ Failed to clear session state: {str(e)}")
