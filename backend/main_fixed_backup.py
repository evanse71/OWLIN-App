...
async def process_file_with_real_ocr(file_path: Path, original_filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            from ocr.enhanced_line_item_extractor import EnhancedLineItemExtractor
            from ocr.ocr_engine import preprocess_image
            from db_manager import save_invoice_to_db, save_uploaded_file_to_db
        except ImportError as import_error:
            logger.error(f"❌ Failed to import OCR modules: {import_error}")
        ...
        try:
            ...
            logger.error(f"❌ Failed to initialize OCR engine: {engine_error}")
            # Fallback to basic text processing
            return await process_file_with_basic_ocr(file_path, original_filename)
        ...
        try:
            ocr_results = await ocr_engine.process_document(str(file_path))
            logger.info(f"📊 OCR processing completed: {len(ocr_results) if ocr_results else 0} results")
        ...
        if not ocr_results or len(ocr_results) == 0:
            logger.warning("❌ No OCR results obtained, using fallback")
            return await process_file_with_basic_ocr(file_path, original_filename)
        
        # Extract line items
        try:
            line_extractor = EnhancedLineItemExtractor()
            line_items = line_extractor.extract_line_items(ocr_results)
            logger.info(f"📋 Line item extraction completed: {len(line_items)} items")
        ...
        logger.info(f"📊 Text processing completed: {word_count} words, {avg_confidence:.2f} avg confidence")
        
        # Extract basic invoice data
        extracted_data = extract_invoice_data_from_text(all_text)
...
async def process_file_with_basic_ocr(file_path: Path, original_filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if file_extension in {'.txt', '.md'}:
            ...
            return {
                "confidence": 0.85,  # High confidence for text files
                "supplier_name": extracted_data["supplier_name"],
                "invoice_number": extracted_data["invoice_number"],
                "total_amount": extracted_data["total_amount"],
                "invoice_date": extracted_data["invoice_date"],
        ...
        # Handle image files with basic Tesseract
            ...
            try:
                ...
                if text_content.strip():
                    ...
                    return {
                        "confidence": 0.75,  # Medium confidence for image OCR
                        "supplier_name": extracted_data["supplier_name"],
                        "invoice_number": extracted_data["invoice_number"],
                        "total_amount": extracted_data["total_amount"],
                        "invoice_date": extracted_data["invoice_date"],
...
def extract_invoice_data_from_text(text: str) -> Dict[str, Any]:
    ...
    # Pattern 1: Look for "Total:" or "Total Due:" (including VAT)
    ...
    # Pattern 2: Look for "Total (inc. VAT):" or similar
    if total_amount == 0.0:
    ...
    # Pattern 3: Look for currency amounts at the end (largest amount, but not invoice numbers)
    if total_amount == 0.0:
    ...
    # Pattern 4: Look for amounts in the last few lines (where totals usually are)
    if total_amount == 0.0:
        lines = text.split('\n')
    ...
    # Pattern 5: Look specifically for "Total (inc. VAT): £556.20" pattern
    ...
    # Pattern 6: Look for "Total:" followed by amount (usually includes VAT)
...
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        return {
            ...
            # ✅ CRITICAL: Nest data under parsed_data as frontend expects
            "parsed_data": {
                "confidence": ocr_data["confidence"],
                "supplier_name": ocr_data["supplier_name"],
                "invoice_number": ocr_data["invoice_number"],
