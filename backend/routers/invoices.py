...
@router.get("")
def list_invoices():
    rows = db.fetch_all("SELECT id, supplier, invoice_date, status, currency FROM invoices ORDER BY ROWID DESC", ())
    out = []
    for r in rows:
        pages = db.fetch_all("SELECT page_no FROM invoice_pages WHERE invoice_id=? ORDER BY page_no ASC", (r["id"],))
        out.append({**dict(r), "pages":[p["page_no"] for p in pages], "page_count": len(pages)})
    return {"items": out}

@router.get("/{invoice_id}")
def get_invoice(invoice_id: str):
    inv = db.fetch_one("SELECT id, supplier, invoice_date, status, currency FROM invoices WHERE id=?", (invoice_id,))
    if not inv: raise HTTPException(404, "invoice not found")
    pages = db.fetch_all("SELECT page_no FROM invoice_pages WHERE invoice_id=? ORDER BY page_no ASC", (invoice_id,))
...
def create_manual(inv: InvoiceManualIn):
    ...
    for li in inv.line_items or []:
        ...
        db.execute("INSERT INTO invoice_line_items (id, invoice_id, description, quantity, unit_price, total, uom, vat_rate, source) VALUES (?,?,?,?,?,?,?,?,?)",
...
def get_line_items(invoice_id: str):
    rows = db.fetch_all("SELECT id, description, quantity, unit_price, total, uom, vat_rate, source FROM invoice_line_items WHERE invoice_id=? ORDER BY id ASC", (invoice_id,))
...
def add_line_items(invoice_id: str, items: List[Dict[str,Any]]):
    for li in items or []:
        ...
        db.execute("INSERT INTO invoice_line_items (id, invoice_id, description, quantity, unit_price, total, uom, vat_rate, source) VALUES (?,?,?,?,?,?,?,?,?)",
                   (str(uuid.uuid4()), invoice_id, li.get("description"), q, up, q*up, li.get("uom"), float(li.get("vat_rate") or 0), li.get("source") or "manual"))
    return {"ok": True}

@router.post("/{invoice_id}/rescan")
def rescan_invoice(invoice_id: str):
    inv = db.fetch_one("SELECT document_id FROM invoices WHERE id=?", (invoice_id,))
    if not inv or not inv["document_id"]:
        raise HTTPException(400, "no source document")
    doc = db.fetch_one("SELECT path FROM documents WHERE id=?", (inv["document_id"],))
    ...
    for p in pages:
        ...
        for li in parsed.get("line_items", []):
            ...
            db.execute(
              ...
              (str(uuid.uuid4()), invoice_id, li.get("description"), q, up, q*up, li.get("uom"), float(li.get("vat_rate") or 0), li.get("source") or "ocr")
            )
    return {"ok": True}

@router.put("/{invoice_id}/line-items/{line_id}")
def update_line_item(invoice_id: str, line_id: str, body: Dict[str,Any] = Body(...)):
    row = db.fetch_one("SELECT id FROM invoice_line_items WHERE id=? AND invoice_id=?", (line_id, invoice_id))
...
def delete_line_item(invoice_id: str, line_id: str):
    db.execute("DELETE FROM invoice_line_items WHERE id=? AND invoice_id=?", (line_id, invoice_id))
    return {"ok": True}