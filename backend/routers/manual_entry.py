...
from ..services.pack_math import calc_line_totals
...
# ---- SCHEMA ENSURE: idempotent, runs once per process ----
def ensure_schema():
    with sqlite3.connect(DB_PATH) as conn:
        c = conn.cursor()
        # invoices table must have columns
        c.execute("PRAGMA table_info(invoices)")
        cols = {row[1] for row in c.fetchall()}
        if "entry_mode" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN entry_mode TEXT DEFAULT 'ocr'")
        if "currency" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN currency TEXT DEFAULT 'GBP'")
        if "total_net" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN total_net TEXT")
        if "total_vat" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN total_vat TEXT")
        if "total_gross" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN total_gross TEXT")
        if "notes" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN notes TEXT")
        if "meta_json" not in cols:
            c.execute("ALTER TABLE invoices ADD COLUMN meta_json TEXT")

        # delivery_notes table must have columns
        c.execute("PRAGMA table_info(delivery_notes)")
        cols = {row[1] for row in c.fetchall()}
        if "entry_mode" not in cols:
            c.execute("ALTER TABLE delivery_notes ADD COLUMN entry_mode TEXT DEFAULT 'ocr'")
        if "currency" not in cols:
            c.execute("ALTER TABLE delivery_notes ADD COLUMN currency TEXT DEFAULT 'GBP'")
        if "notes" not in cols:
            c.execute("ALTER TABLE delivery_notes ADD COLUMN notes TEXT")
        if "meta_json" not in cols:
            c.execute("ALTER TABLE delivery_notes ADD COLUMN meta_json TEXT")
        ...
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_links_invoice ON invoice_delivery_links(invoice_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_links_dn ON invoice_delivery_links(delivery_note_id)")
        conn.commit()
