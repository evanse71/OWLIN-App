...
import { DocumentQueueItem } from '@/services/api';
...
const DocumentQueueCard: React.FC<DocumentQueueCardProps> = ({ 
  ...
  const needsReview = document.status === 'pending' || document.status === 'failed' || document.confidence < 0.7;
  const hasLowConfidence = document.confidence < 0.7;
      ...
      <div className="absolute top-3 right-3 flex space-x-1">
        {hasLowConfidence && (
          <div 
            ...
            title={`Low OCR confidence: ${Math.round(document.confidence * 100)}%`}
          />
        )}
        {hasError && (
          <div 
