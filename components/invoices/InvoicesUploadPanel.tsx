...
const InvoicesUploadPanel: React.FC<InvoicesUploadPanelProps> = ({ onDeliveryNotesUpdate, onInvoicesUpdate }) => {
  ...
  const handleRetryOCR = async (documentId: string) => {
    // Find the document to retry
    const docToRetry = documents.find(doc => doc.id === documentId);
    if (!docToRetry) {
      console.warn(`Document with ID ${documentId} not found for retry`);
      return;
    }

    // For now, just show a message that retry is not fully implemented
    console.log('Retry requested for document:', docToRetry);
    showError('Retry functionality is not fully implemented yet. Please re-upload the file.');
  };
            ...
            <div className="space-y-3">
              {documents.map((doc) => (
                <DocumentCard
                  ...
                  onRetry={() => handleRetryOCR(doc.id)}
                  onCancel={() => handleCancelDocument(doc.id)}
                  isUtilityInvoice={doc.parsedData?.delivery_note_required === false}
                  deliveryNoteRequired={doc.parsedData?.delivery_note_required !== false}
                  multipleInvoices={doc.multipleInvoices || false}
                  invoiceCount={doc.invoiceCount || 1}
                />
              ))}
            </div>
          </div>
