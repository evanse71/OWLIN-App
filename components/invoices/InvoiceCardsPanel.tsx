...
const InvoiceCardsPanel: React.FC<InvoiceCardsPanelProps> = ({
  ...
  const getVisibleDocuments = () => {
    ...
    if (showScannedOnly) {
      ...
      visibleDocs = documents.scannedAwaitingMatch.filter(doc => 
        doc.status === 'scanned'
      );
    } else if (showMatchedOnly) {
      // Show only matched documents
      visibleDocs = documents.matchedDocuments;
      ...
      visibleDocs = documents.scannedAwaitingMatch;
    }
    ...
    console.log("All scannedAwaitingMatch documents:", documents.scannedAwaitingMatch.length);
    console.log("All matchedDocuments:", documents.matchedDocuments.length);
    console.log("Visible documents after filtering:", visibleDocs.length);
    
    // Log status breakdown
    const statusBreakdown = documents.scannedAwaitingMatch.reduce((acc, doc) => {
    ...
    const scannedDocs = documents.scannedAwaitingMatch.filter(doc => doc.status === 'scanned');
  ...
  const handleDocumentClick = (document: Invoice | DeliveryNote) => {
    if ('invoice_number' in document) {
      ...
      const matchingDeliveryNote = documents.matchedDocuments.find(doc => 
        'delivery_note_number' in doc && doc.invoice?.id === document.id
      );
      ...
      const matchingInvoice = documents.matchedDocuments.find(doc => 
        'invoice_number' in doc && doc.delivery_note?.id === document.id
      );
              ...
              if ('invoice_number' in document) {
                ...
                return (
                  <InvoiceCardAccordion
                    key={invoice.id}
                    invoice={invoice}
                    deliveryNote={matchingDeliveryNote}
