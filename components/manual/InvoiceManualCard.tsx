...
export default function InvoiceManualCard({ variant="card", onSaved, onCancel }: Props) {
  ...
  const onSubmit = async (data: FormData, clearAfter = true) => {
    try {
      ...
      else toast({ title: "Success", description: `Invoice saved: ${res.id}` });
      ...
      if (/409/.test(msg) || /already exists/i.test(msg)) {
        form.setError("invoice_ref", { type: "manual", message: "Already used. Pick a unique reference." });
        return;
      }
